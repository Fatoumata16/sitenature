...
function econature_lite_font_url(){
		...
		*/
		$robo = _x('on', 'Roboto font:on or off','econature-lite');
		
		/* Translators: If there are any character that are
		* not supported by Roboto Condensed, translate this to off, do not
		* translate into your own language.
		*/
		$roco = _x('on', 'Roboto Condensed font:on or off','econature-lite');
		
		if( 'off' !== $robo || 'off' !== $roco ){
			$font_family = array();
			
			if('off' !== $robo){
				$font_family[] = 'Roboto:300,400,500,600,700';
			}
			if('off' !== $roco){
...
function econature_lite_scripts() {
	...
	wp_enqueue_script( 'econature-lite-navigation', get_template_directory_uri() . '/js/navigation.js', array('jquery'), '20190715', true );
	...
	wp_localize_script( 'econature-lite-navigation', 'NavigationScreenReaderText', array() );
}
